...
- To give a method real behaviour, add a mapping with a higher priority (a lower number) whose
  `urlPath` is `/api/method/...` with the method name in place of `...` (e.g.
  `hrms.api.mobile_v1.get_dashboard_overview`), and a response body in the shape the HRIS backend
  returns. Use invented data only: never copy real employee records here.
